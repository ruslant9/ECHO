  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      >
        ...
        {/* Иконка теперь поворачивается, когда меню открыто */}
        <ChevronDown 
          size={20} 
      ...
      {/* --- ВЫПАДАЮЩЕЕ МЕНЮ (ТЕПЕРЬ ОТКРЫВАЕТСЯ ВВЕРХ) --- */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            // ИЗМЕНЕНИЯ В АНИМАЦИИ: y: 10 (сдвиг снизу вверх)
            initial={{ opacity: 0, y: 10, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 10, scale: 0.95 }}
            transition={{ duration: 0.2, ease: "easeOut" }}
            // ИЗМЕНЕНИЯ В CSS: bottom-full вместо top-full, mb-2 вместо mt-2
            className={`absolute bottom-full left-0 right-0 mb-2 z-50 rounded-2xl shadow-xl overflow-hidden border backdrop-blur-xl
              ${isDarkMode 
