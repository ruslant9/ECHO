...
export default function PostCard({ post, currentUserId, onEdit, onUpdate, highlightCommentId, isSinglePostView, isScheduledView = false }: PostCardProps) {
  ...
  <div className="px-4 pb-4">
    <div className={`p-3 rounded-[20px] border flex flex-col items-center gap-3 transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/10'}`}>
      ...
      {/* --- ОБНОВЛЕННЫЙ ЗАГОЛОВОК С СЫЛКОЙ --- */}
      <div className="w-full flex items-center justify-between gap-2 mb-1 px-2">
        <Link 
          href={`/dashboard/vibes?vibeId=${displayContent.vibe.id}`}
          className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-lime-500 hover:text-lime-400 transition-colors group/vibelink"
        >
          ...
          {/* Иконка стрелочки, которая появляется при наведении */}
          <Send size={10} className="opacity-0 -translate-x-2 group-hover/vibelink:opacity-100 group-hover/vibelink:translate-x-0 transition-all" />
        </Link>
