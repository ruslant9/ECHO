...
import { 
  ...
  ChevronDown, LogOut, Settings, Shield, Clapperboard, Plus, Music
} from 'lucide-react';
...
export default function Header() {
  ...
  // Определяем, находимся ли мы на странице вайбов (там всегда темный фон)
  ...
  const { data, refetch } = useQuery(GET_HEADER_DATA, { 
      pollInterval: 5000, 
  ...
  // Определяем, должен ли контент быть светлым (для темного фона)
  const isLightContent = (mounted && isDarkMode) || isVibesPage;
  
  // Класс для кнопки-галочки
  // Если isLightContent (Темная тема) -> Черный круг (zinc-900), белая иконка
  // Если !isLightContent (Светлая тема) -> Белый круг, серая иконка (zinc-500)
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .liquid-switcher__icon {
          display: block;
          transition: scale 200ms cubic-bezier(0.5, 0, 0, 1);
        }

        .liquid-switcher__option:hover .liquid-switcher__icon { scale: 1.2; }
        .liquid-switcher__option[data-active="true"] .liquid-switcher__icon { scale: 1; }
        ...
        .liquid-switcher[data-active-index="0"]::after { translate: 0 0; animation: scaleToggle 440ms ease; }
        .liquid-switcher[data-active-index="1"]::after { translate: 76px 0; animation: scaleToggle 440ms ease; }
        .liquid-switcher[data-active-index="2"]::after { translate: 152px 0; animation: scaleToggle 440ms ease; }
        .liquid-switcher[data-active-index="3"]::after { translate: 228px 0; animation: scaleToggle 440ms ease; }
        .liquid-switcher[data-active-index="4"]::after { translate: 304px 0; animation: scaleToggle 440ms ease; }
        /* Новый пункт */
        .liquid-switcher[data-active-index="5"]::after { translate: 380px 0; animation: scaleToggle 440ms ease; } 

        @keyframes scaleToggle { 0% { scale: 1 1; } 50% { scale: 1.15 1; } 100% { scale: 1 1; } }
      `}} />
     ...
     <header className={`fixed top-[26px] left-0 right-0 z-50 w-full transition-colors duration-300 pointer-events-none ${bgClass}`}>
                      ...
                      {isDropdownOpen && (
                        ...
                        >
                          ...
                          <div className="p-3 flex flex-col gap-2">
                            ...
                            {isAdmin && (
                              <Link href="/admin" onClick={() => setIsDropdownOpen(false)} className={`w-full flex items-center gap-3 px-3 py-3 rounded-2xl text-sm font-medium transition-colors text-blue-600 hover:bg-blue-50 dark:text-blue-500 dark:hover:bg-blue-500/10`}>
