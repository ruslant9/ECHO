...
import { Plus, Edit2, Trash2, Mic2, Disc3, Music, Loader, Search, X, AlertTriangle } from 'lucide-react';
...
// Массовое удаление
const DELETE_ALL_ARTISTS = gql`
  mutation AdminDeleteAll {
    adminDeleteAllArtists
  }
`;
...
export default function AdminMusicPanel() {
  ...
  const [deleteAllArtists, { loading: isDeletingAll }] = useMutation(DELETE_ALL_ARTISTS);
  ...
  const handleDeleteClick = (type: TabType, item: any) => {
    ...
    if (type === 'artists') {
      ...
      message = `Внимание! Артист «${item.name}», а также ВСЕ его альбомы и треки будут удалены из базы и Cloudinary.`;
    } else if (type === 'albums') {
      title = 'Удалить альбом?';
      message = `Альбом «${item.title}» и все треки внутри него будут удалены отовсюду.`;
    } else {
      title = 'Удалить трек?';
      message = `Трек «${item.title}» будет полностью удален.`;
    }
  ...
  const handleDeleteAllClick = () => {
    setConfirmModal({
        ...
        type: 'all',
        id: null,
        title: 'УДАЛИТЬ ВООБЩЕ ВСЁ?',
        message: 'Это действие удалит ВСЕХ артистов, альбомы и аудиофайлы из базы данных и ИЗ ОБЛАКА Cloudinary. Восстановление невозможно.'
    });
  };
  ...
  const executeDelete = async () => {
    try {
      if (confirmModal.type === 'all') {
        ...
        setToast({ message: 'Вся медиатека очищена', type: 'success' });
      } else if (confirmModal.type === 'artists' && confirmModal.id) {
        await deleteArtist({ variables: { id: confirmModal.id } });
        setToast({ message: 'Артист удален', type: 'success' });
      } else if (confirmModal.type === 'albums' && confirmModal.id) {
        await deleteAlbum({ variables: { id: confirmModal.id } });
        setToast({ message: 'Альбом удален', type: 'success' });
      } else if (confirmModal.type === 'tracks' && confirmModal.id) {
        await deleteTrack({ variables: { id: confirmModal.id } });
        setToast({ message: 'Трек удален', type: 'success' });
      }
      refetchAll();
    } catch (e: any) {
            ...
            <div key={artist.id} className={cardClass}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-12 h-12 rounded-lg bg-zinc-800 shrink-0 overflow-hidden flex items-center justify-center border border-zinc-700">
                  {artist.avatar ? <img src={getAvatarUrl(artist.avatar)!} className="w-full h-full object-cover" alt={artist.name} /> : <Mic2 size={24} className="text-zinc-600" />}
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-sm truncate">{artist.name}</h3>
                  <p className="text-xs text-zinc-500 truncate">{artist.bio || 'Yandex Import'}</p>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {albums.map((album: any) => (
            <div key={album.id} className={cardClass}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-12 h-12 rounded-lg bg-zinc-800 shrink-0 overflow-hidden flex items-center justify-center border border-zinc-700">
                  {album.coverUrl ? <img src={getAvatarUrl(album.coverUrl)!} className="w-full h-full object-cover" alt={album.title} /> : <Disc3 size={24} className="text-zinc-600" />}
                </div>
                <div className="min-w-0 flex-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {tracks.map((track: any) => (
            <div key={track.id} className={cardClass}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-12 h-12 rounded-lg bg-zinc-800 shrink-0 overflow-hidden flex items-center justify-center border border-zinc-700">
                  {track.coverUrl ? <img src={getAvatarUrl(track.coverUrl)!} className="w-full h-full object-cover" alt={track.title} /> : <Music size={24} className="text-zinc-600" />}
                </div>
                <div className="min-w-0 flex-1">
    ...
    <div className={`p-6 rounded-3xl border ${isDarkMode ? 'bg-zinc-900/50 border-zinc-800' : 'bg-white border-zinc-200'}`}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .admin-liquid-switcher {
          position: relative; z-index: 1; display: flex; align-items: center; gap: 4px; height: 52px; padding: 6px; border-radius: 99em;
        ...
        .admin-liquid-option {
          ...
          height: 100%; padding: 0 20px; border-radius: 99em; font-size: 14px; font-weight: 700; cursor: pointer; transition: color 160ms;
        }
        .admin-liquid-option[data-active="true"] { color: var(--c-content); cursor: default; }
        .admin-liquid-blob { border-radius: 99em; background-color: color-mix(in srgb, var(--c-glass) 36%, transparent); box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--c-light) 10%, transparent); }
      `}} />
      ...
      <AdminArtistEditor isOpen={editorState.isOpen && editorState.type === 'artists'} onClose={() => setEditorState({ isOpen: false, type: null, data: null })} artist={editorState.data} onSuccess={(msg: string) => { setToast({ message: msg, type: 'success' }); refetchAll(); setEditorState({ isOpen: false, type: null, data: null }); }} />
      <AdminAlbumEditor isOpen={editorState.isOpen && editorState.type === 'albums'} onClose={() => setEditorState({ isOpen: false, type: null, data: null })} album={editorState.data} artists={artistsData?.adminGetAllArtists || []} allTracks={tracksData?.adminGetAllTracks || []} onSuccess={(msg: string) => { setToast({ message: msg, type: 'success' }); refetchAll(); setEditorState({ isOpen: false, type: null, data: null }); }} />
      <AdminTrackEditor isOpen={editorState.isOpen && editorState.type === 'tracks'} onClose={() => setEditorState({ isOpen: false, type: null, data: null })} track={editorState.data} artists={artistsData?.adminGetAllArtists || []} albums={albumsData?.adminGetAllAlbums || []} onSuccess={(msg: string) => { setToast({ message: msg, type: 'success' }); refetchAll(); setEditorState({ isOpen: false, type: null, data: null }); }} />

      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div className="admin-liquid-switcher" style={liquidGlassStyles}>
                {[{ id: 'artists', label: 'Артисты', icon: Mic2 }, { id: 'albums', label: 'Альбомы', icon: Disc3 }, { id: 'tracks', label: 'Треки', icon: Music }].map((tab) => (
                    <button key={tab.id} onClick={() => { setActiveTab(tab.id as TabType); setSearchQuery(''); }} className="admin-liquid-option" data-active={activeTab === tab.id}>
                        {activeTab === tab.id && <motion.div layoutId="admin-music-tab-blob" className="admin-liquid-blob absolute inset-0 z-0" transition={{ type: "spring", stiffness: 400, damping: 30 }} />}
                        <div className="relative z-10 flex items-center gap-2"><tab.icon size={18} /><span>{tab.label}</span></div>
                    </button>
                ))}
            </div>
            ...
            <div className="relative w-full md:w-64">
                ...
                <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Найти..." className={`w-full pl-10 pr-10 py-3 rounded-2xl outline-none transition-all border text-sm ${isDarkMode ? 'bg-zinc-800 border-zinc-700 text-white focus:border-lime-500' : 'bg-zinc-50 border-zinc-200 focus:border-lime-500 text-zinc-900 shadow-sm'}`} />
            </div>
            ...
            <button 
                ...
                className="flex items-center gap-2 px-5 py-3 bg-red-500/10 text-red-500 border border-red-500/20 rounded-full text-xs font-black hover:bg-red-500 hover:text-white transition-all cursor-pointer disabled:opacity-50"
            >
                {isDeletingAll ? <Loader size={16} className="animate-spin" /> : <Trash2 size={16} />}
                УДАЛИТЬ ВСЕХ АРТИСТОВ
            </button>
        </div>

        <button onClick={() => setEditorState({ isOpen: true, type: activeTab, data: null })} className="flex items-center gap-2 px-5 py-3 bg-zinc-900 text-white dark:bg-white dark:text-black rounded-full text-sm font-bold hover:scale-105 active:scale-95 transition-transform cursor-pointer shadow-lg w-full md:w-auto justify-center">
          <Plus size={18} /> Добавить {activeTab === 'artists' ? 'артиста' : activeTab === 'albums' ? 'альбом' : 'трек'}
        </button>
      </div>
