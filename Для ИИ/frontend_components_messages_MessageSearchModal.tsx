      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            // ДОБАВЛЕНО: Явное указание цвета текста (text-zinc-100 для темной, text-zinc-900 для светлой)
            className={`w-full max-w-2xl rounded-[28px] overflow-hidden shadow-2xl border flex flex-col max-h-[70vh]
              ${isDarkMode ? 'bg-zinc-900/90 border-white/10 text-zinc-100' : 'bg-white/90 border-zinc-200 text-zinc-900'}`}
          >
            {/* Заголовок */}
            <div className={`flex items-center justify-between p-5 border-b ${isDarkMode ? 'border-white/10' : 'border-zinc-200'}`}>
              ...
              <button
                onClick={onClose}
                className={`p-2 rounded-full transition-colors cursor-pointer ${isDarkMode ? 'hover:bg-white/10' : 'hover:bg-zinc-100 text-zinc-500 hover:text-zinc-900'}`}
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-5">
              <div className={`relative flex items-center px-4 py-3 rounded-xl border ${isDarkMode ? 'bg-zinc-800 border-zinc-700' : 'bg-zinc-50 border-zinc-200'}`}>
                ...
                <input
                  ...
                  // ДОБАВЛЕНО: Цвет плейсхолдера
                  className={`bg-transparent w-full outline-none text-sm ${isDarkMode ? 'placeholder:text-zinc-500' : 'placeholder:text-zinc-400'}`}
                />
                {results.length > 0 && (
            ...
            {results.length > 0 && (
              <div className={`px-5 py-3 border-t flex items-center gap-4 text-xs ${isDarkMode ? 'border-white/10 text-zinc-500' : 'border-zinc-200 text-zinc-500'}`}>
                <div className="flex items-center gap-1">
                  <ArrowUp size={14} /> <ArrowDown size={14} /> – навигация
                </div>
                <div className="flex items-center gap-1">
                  <kbd className={`px-2 py-1 rounded font-sans ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100 text-zinc-700'}`}>Enter</kbd> – перейти
                </div>
              </div>
            )}
          </motion.div>
